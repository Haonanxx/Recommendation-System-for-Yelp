    ...
    try:
        for key, value in values[0].items():
            ...
            #find N neighbors
                ...
                try:
                    N_lst.append([0.5, (0.3*usr_avg[pred_pairs[0][0]]+0.7*bus_avg[pred_pairs[0][1]])])
                except:
        ...
        l=len(N_lst)
        if l>=25:
            N_neighbors = sorted(N_lst, reverse=True)[:23]
        elif l>=6:
            N_neighbors = sorted(N_lst, reverse=True)[:(l-1)]
        else:
        ...
        try:
        #    #print(pred_pairs)
            return (pred_pairs[0][0], pred_pairs[0][1], (0.3*usr_avg[pred_pairs[0][0]]+0.7*bus_avg[pred_pairs[0][1]]))
        except:
            #print(pred_pairs)
            return(pred_pairs[0][0], pred_pairs[0][1], 3.21)
